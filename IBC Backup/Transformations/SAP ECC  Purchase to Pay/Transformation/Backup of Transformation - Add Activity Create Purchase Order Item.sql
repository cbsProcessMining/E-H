...
SELECT DISTINCT 
    ...
    ,CASE
        WHEN CDHDR_POS.UDATE IS NOT NULL THEN CAST(CDHDR_POS.UDATE AS DATE) + CAST(CDHDR_POS.UTIME AS TIME)
        WHEN CDHDR_HEAD.UDATE IS NOT NULL THEN CAST(CDHDR_HEAD.UDATE AS DATE) + CAST(CDHDR_HEAD.UTIME AS TIME)
        ELSE CAST(E.AEDAT AS DATE) + CAST('00:00:01' AS TIME)
    END AS "EVENTTIME"
    ,500 AS "_SORTING"
    ,COALESCE(CDHDR_POS.USERNAME, CDHDR_HEAD.USERNAME, E.ERNAM) AS "USER_NAME"
    ,CASE
        WHEN CDHDR_POS.USERNAME IS NOT NULL THEN USR02_POS.USTYP
        WHEN CDHDR_HEAD.USERNAME IS NOT NULL THEN USR02_HEAD.USTYP
...
FROM
	...
	LEFT JOIN CDPOS AS CDPOS_POS ON 1=1
        AND CDPOS_POS."SCHEMA" = E."SCHEMA" -- global job 
		AND CDPOS_POS.TABKEY = E.MANDT || E.EBELN || E.EBELP  -- globale job Anpassung, da vorher auf Case Key eingeschränkt wurde
